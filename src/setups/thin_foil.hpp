...
void setup(Params &params) {
  ...
  // sphere
  auto profile = [n0](double x, double y, double z) -> double {
    const double xc = (x * 3 - 1.5);
    const double R  = xc * xc + (y - 0.5) * (y - 0.5) + (z - 0.5) * (z - 0.5);
    if (R < 0.25 * 0.25) {
      return n0;
    } else {
      return 0;
  ...
  // Hydrogen Plasma
  params.add_species("electron", 1, -1, temperature, profile, {0, 0, 0}, 8, "random", "cell");
  params.add_species("proton", 1836.125, 1, temperature, profile, {0, 0, 0}, 8, "electron", "cell");
