...
#pragma omp simd
    ...
    for (unsigned int part = 0; part < n_particles; part++) {
      ...
      const auto ixn = (patch.particles_m[is].x_h(part) - xmin) * inv_dx_m;
      const auto iyn = (patch.particles_m[is].y_h(part) - ymin) * inv_dy_m;
      const auto izn = (patch.particles_m[is].z_h(part) - zmin) * inv_dz_m;
...
//! \brief Interpolation operator at the patch level :
//! interpolate EM fields from global grid for each particle
auto interpolate_bin(Params &params, ElectroMagn &em, Particles<double> &particles, int is, int init, int end)
  -> void {
...
#pragma omp simd
  ...
  for (int part = init; part < end; part++) {
    ...
    // // Calculate normalized positions
    const auto ixn = (particles.x_h(part) - xmin) * inv_dx_m;
    const auto iyn = (particles.y_h(part) - ymin) * inv_dy_m;
    const auto izn = (particles.z_h(part) - zmin) * inv_dz_m;
    ...
    // Ex (d, p , p)
    {
      ...
      const auto v0 = v00 * (1 - dist_y_p) + v10 * dist_y_p;
      const auto v1 = v01 * (1 - dist_y_p) + v11 * dist_y_p;
      particles.Ex_(part) = v0 * (1 - dist_z_p) + v1 * dist_z_p;
    }
    ...
    // Ey (p, d, p)
    {
      ...
      particles.Ey_(part) = v0 * (1 - dist_z_p) + v1 * dist_z_p;
    }
    ...
    // Ez (p, p, d)
    {
      ...
      particles.Ez_(part) = v0 * (1 - dist_z_d) + v1 * dist_z_d;
    }
...
//     const int n_particles                = patch.particles_m[is].size();
//     const mini_float inv_cell_volume_x_q = params.inv_cell_volume *
//     patch.particles_m[is].charge_m;
...
// _______________________________________________________
auto solve_maxwell_ampere_ex_2d(Params &params,
                                Timers &timers,
                                Profiler &profiler,
                                ElectroMagn &em,
                                int ix,
...
auto solve_maxwell_ampere_ey_2d(Params &params,
                                Timers &timers,
                                Profiler &profiler,
                                ElectroMagn &em,
                                int ix,
...
auto solve_maxwell_ampere_ez_2d(Params &params,
                                Timers &timers,
                                Profiler &profiler,
                                ElectroMagn &em,
                                int ix,
...
auto solve_maxwell_faraday_bx_2d(Params &params,
                                 Timers &timers,
                                 Profiler &profiler,
                                 ElectroMagn &em,
                                 int ix,
...
auto solve_maxwell_faraday_by_2d(Params &params,
                                 Timers &timers,
                                 Profiler &profiler,
                                 ElectroMagn &em,
                                 int ix,
...
auto solve_maxwell_faraday_bz_2d(Params &params,
                                 Timers &timers,
                                 Profiler &profiler,
                                 ElectroMagn &em,
                                 int ix,
...
//! \param[in] Patch & patch - current patch
// ____________________________________________________________________________
auto local2global(ElectroMagn &em, Patch &patch, std::mutex &mutex) -> void {
    ...
    for (int ix = 0; ix < patch.vec_Jx_m[0].nx(); ix++) {
      for (int iy = 0; iy < patch.vec_Jx_m[0].ny(); iy++) {
        for (int iz = 0; iz < patch.vec_Jx_m[0].nz(); iz++) {
          ...
          const int index = em.Jx_m.index(i_global_d + ix, j_global_p + iy, k_global_p + iz);

          // std::atomic<mini_float *> J(em.Jx_m.get_raw_pointer(i_global_d + ix, j_global_p + iy,
          // k_global_p + iz, minipic::host));

          // std::atomic<mini_float *> J(em.Jx_m.get_raw_pointer( minipic::host) + index);

          // J.fetch_add(patch.vec_Jx_m[0](ix, iy, iz));
    ...
    for (int ix = 0; ix < patch.vec_Jy_m[0].nx(); ix++) {
      for (int iy = 0; iy < patch.vec_Jy_m[0].ny(); iy++) {
        for (int iz = 0; iz < patch.vec_Jy_m[0].nz(); iz++) {
          ...
          // const int index = em.Jy_m.index(i_global_p + ix, j_global_d + iy, k_global_p + iz);

          // std::atomic<mini_float *> J(em.Jy_m.get_raw_pointer(minipic::host) + index);

          // J.fetch_add(patch.vec_Jy_m[0](ix, iy, iz), std::memory_order_relaxed);
    ...
    for (int ix = 0; ix < patch.vec_Jz_m[0].nx(); ix++) {
      for (int iy = 0; iy < patch.vec_Jz_m[0].ny(); iy++) {
        for (int iz = 0; iz < patch.vec_Jz_m[0].nz(); iz++) {
          ...
          // const int index = em.Jz_m.index(i_global_p + ix, j_global_p + iy, k_global_d + iz);

          // std::atomic<mini_float *> J(em.Jz_m.get_raw_pointer(i_global_p + ix, j_global_p + iy,
          // k_global_d + iz, minipic::host));

          // J.fetch_add(patch.vec_Jz_m[0](ix, iy, iz), std::memory_order_relaxed);
...
auto particle_binning(Params &params,
                      Timers &timers,
                      Profiler &profiler,
...
auto particle_cloud(Params &params,
                    Timers &timers,
                    Profiler &profiler,
                    std::vector<Patch> &patches_,
                    unsigned int it) -> void {
...
auto particle_scalars(Params &params,
                      Timers &timers,
                      Profiler &profiler,
                      std::vector<Patch> &patches_,
                      unsigned int it) -> void {
...
auto particle_diags(Params &params,
                    Timers &timers,
                    Profiler &profiler,
                    std::vector<Patch> &patches_,
                    unsigned int it) -> void {
  ...
  particle_scalars(params, timers, profiler, patches_, it);
};
...
auto diags_fields(Params &params,
  ...
  if (!(it % params.field_diagnostics_period)) {
    ...
    Diags::fields(params, em, it, params.field_diagnostics_format);
...
unsigned int get_total_number_of_particles(std::vector<Patch> &patches_) {
...
auto terminal_print(Params &params,
                    Timers &timers,
                    Profiler &profiler,
                    std::vector<Patch> &patches_,
                    unsigned int it) -> void {
  if (!(it % params.print_period)) {
    ...
    std::cout << " | " << std::scientific << std::setprecision(2) << std::setw(8) << elapsed_time;
    std::cout << " | " << std::scientific << std::setprecision(2) << std::setw(9) << remaining_time;
    std::cout << " | " << std::endl;
  }
};
...
auto reset_current(Params params, Timers &timers, Profiler &profiler, ElectroMagn &em) -> void {

  if (params.current_projection || params.n_particles > 0) {

    DEBUG("start reset current");

    timers.start(timers.projection);
    profiler.start(RESET);

    em.reset_currents(minipic::host);

    timers.stop(timers.projection);
    profiler.stop();
