...
public:
  ...
  */
  ...
  void allocate(const Params &params, Backend &backend) {
    ...
    Jx_m.allocate(nx_d_m, ny_p_m + 2, nz_p_m + 2, backend, 0.0, 1, 0, 0, 0, 0, 0, "Jx");
    Jy_m.allocate(nx_p_m + 2, ny_d_m, nz_p_m + 2, backend, 0.0, 0, 1, 0, 0, 0, 0, "Jy");
    Jz_m.allocate(nx_p_m + 2, ny_p_m + 2, nz_d_m, backend, 0.0, 0, 0, 1, 0, 0, 0, "Jz");
    ...
    Ex_m.allocate(nx_d_m, ny_p_m, nz_p_m, backend, params.E0_[0], 1, 0, 0, 0, 0, 0, "Ex");
    Ey_m.allocate(nx_p_m, ny_d_m, nz_p_m, backend, params.E0_[1], 0, 1, 0, 0, 0, 0, "Ey");
    Ez_m.allocate(nx_p_m, ny_p_m, nz_d_m, backend, params.E0_[2], 0, 0, 1, 0, 0, 0, "Ez");
    DEBUG("End Allocate electric fields");

    DEBUG("Start Allocate magnetic fields");
    Bx_m.allocate(nx_p_m, ny_d_m, nz_d_m, backend, params.B0_[0], 0, 1, 1, 0, 0, 0, "Bx");
    By_m.allocate(nx_d_m, ny_p_m, nz_d_m, backend, params.B0_[1], 1, 0, 1, 0, 0, 0, "By");
    Bz_m.allocate(nx_d_m, ny_d_m, nz_p_m, backend, params.B0_[2], 1, 1, 0, 0, 0, 0, "Bz");
