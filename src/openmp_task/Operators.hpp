    ...
    for (unsigned int part = 0; part < n_particles; part++) {
      ...
      // Bx (p, d, d)
      {
        ...
        const mini_float v11 =
          em.Bx_m(ixp, iyd + 1, izd + 1) * (1 - dist_x_p) + em.Bx_m(ixp + 1, iyd + 1, izd + 1) * dist_x_p;
        const mini_float v0 = v00 * (1 - dist_y_d) + v10 * dist_y_d;
        const mini_float v1 = v01 * (1 - dist_y_d) + v11 * dist_y_d;
      ...
      // By (d, p, d)
      {
        ...
        const mini_float v11 =
          em.By_m(ixd, iyp + 1, izd + 1) * (1 - dist_x_d) + em.By_m(ixd + 1, iyp + 1, izd + 1) * dist_x_d;
        const mini_float v0 = v00 * (1 - dist_y_p) + v10 * dist_y_p;
        const mini_float v1 = v01 * (1 - dist_y_p) + v11 * dist_y_p;
      ...
      // Bz (d, d, p)
      {
        ...
        const mini_float v11 =
          em.Bz_m(ixd, iyd + 1, izp + 1) * (1 - dist_x_d) + em.Bz_m(ixd + 1, iyd + 1, izp + 1) * dist_x_d;
        const mini_float v0 = v00 * (1 - dist_y_d) + v10 * dist_y_d;
        const mini_float v1 = v01 * (1 - dist_y_d) + v11 * dist_y_d;
...
#pragma omp simd
  ...
  for (int part = init; part < end; part++) {
    ...
    // Bx (p, d, d)
    {
      ...
      particles.Bx_(part) = v0 * (1 - dist_z_d) + v1 * dist_z_d;
    }
    ...
    // By (d, p, d)
    {
      ...
      particles.By_(part) = v0 * (1 - dist_z_d) + v1 * dist_z_d;
    }
    ...
    // Bz (d, d, p)
    {
      ...
      particles.Bz_(part) = v0 * (1 - dist_z_p) + v1 * dist_z_p;
    }
  } // End for each particle
}
